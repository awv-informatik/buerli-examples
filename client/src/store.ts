...
export type GetScene = (model: ClassCAD, productOrSolidId: ObjectID | ObjectID[]) => Promise<THREE.Scene>

export type GetBufferGeom = (model: ClassCAD, productOrSolidId: ObjectID | ObjectID[]) => Promise<THREE.Mesh[]>
...
const storeApi = vanillaCreate<State>(set => ({
  ...
  setModel: (exampleId: string, model: ClassCAD | null) => {
    set(state =>
      produce(state, draft => {
        if (!model) {
...
type State = Readonly<{
  ...
  setModel: (exampleId: string, model: ClassCAD | null) => void
}>
